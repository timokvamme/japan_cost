function [all_results] = CHToolbox_PMF_Analysis_Free_2in1_new(data, legends, showOption, saveOption)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % plots data of 2 conditions on the same picture, free version of CHToolbox_PMF_Analysis, self-assembled data
    % used to fit psychometric function and calculate threshold
    % must contain PMs, ref_intensity and condition name of each PM
    % 22/9/21 add comments and save figure(without showing)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    tic
    
    % draw_on_the_same_figure = 1; % if not, then use sub plot
    ...
    if showOption
        ...
        for i = 1:length(all_results{1})
            ...
            set(gcf, 'Name', 'Psychometric Function Fitting');
            legend([p1, p2], legends, 'Location', 'northwest');
        end
        if ~isempty(saveOption)
            fn = saveOption.fn;
            % wherePlotTo = saveOption.path;
            % id = saveOption.id;
            % save as PNG
            set(gcf, 'PaperPositionMode', 'auto')
            % print([wherePlotTo, id, '_Ebbinghaus'], '-dpng','-r0');
            print(fn, '-dpng','-r0');
        end
    end
