...
class Sound(object):
    ...
    def __init__(self, filename=''):
        """ :recording_filename: a .wav record_file"""
        self.sound = filename
        self._winsound = __import__('winsound')

    def play(self):
        """ plays the sound record_file with low latency"""
...
class csv_writer(object):
    def __init__(self, filename_prefix='', folder='', column_order=[]):
        """
        Take a dictionary and write it to a csv record_file as a row.
    ...
    def _setup_file(self):
        ...
        self.writer = csv.DictWriter(self._file, fieldnames=self.column_order)  # The writer function to csv. It appends a single row to record_file
    ...
    def flush(self):
        """Saves current content to record_file.
...
def getActualFrameRate(frames=1000):
    ...
    # Show a brief instructionText / warning
